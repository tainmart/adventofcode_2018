...
if __name__ == "__main__":
    ...
    for i in range(max_x + 1):
        for j in range(max_y + 1):
            ...
            if min_dists[0][0] != min_dists[1][0]:
                ...
                regions[id_coord] += 1
                # if we are near the edges, mark infinte
    ...
    for i in range(max_x + 1):
        for j in range(max_y + 1):
            shared_region_size += sum(manhattan_dist(x, i, y, j) for (x, y) in coords) < max_distance
    
    print("Size of region containing all locations within max distance: {:d}".format(shared_region_size))